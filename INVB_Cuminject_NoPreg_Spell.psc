ScriptName INVB_Cuminject_NoPreg_Spell extends activemagiceffect

Armor[] Property pArmor_Projector Auto Const Mandatory
Keyword Property kw_Morph Auto
Keyword Property kw_noPreg Auto
form[] Property pArmor_BadBaby Auto Const Mandatory
...
while (u < pArmor_Projector.Length)
	if akActor.GetItemCount(pArmor_Projector[u]) == 1
	...
	If (INVB_Global_BodyType.GetValue() == 0) ; CBBE Original
		...
		while (i < pArmor_BadBaby.Length)
			if akActor.GetItemCount(pArmor_BadBaby[i]) == 1
				akActor.removeitem(pArmor_BadBaby[i], 1)
				akActor.additem(pArmor_baby[i])
			Endif
			i += 1
		endwhile
		
		
		akActor.unequipitem(pArmor_Projector[u])
		akActor.removeitem(pArmor_Projector[u], 1)
	Endif
u += 1
endwhile
EndEvent
